...
void listen_for_updates(SOCKET sock) {
    ...
    while (keep_running) {
        ...
        std::cout << "Client2 received: " << response << std::endl;
...
int main() {
    ...
    // Wait until matched before sending matrices
    while (!matched) {
        std::this_thread::sleep_for(std::chrono::milliseconds(100));
    }

    std::string matrix1 = "GAME_UPDATE|matrix1 from Client2";
    send(sock, matrix1.c_str(), matrix1.length(), 0);
    std::this_thread::sleep_for(std::chrono::seconds(2)); 
    std::string matrix2 = "GAME_UPDATE|matrix2 from Client2";
